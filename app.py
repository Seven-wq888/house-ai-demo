...
# ===================== 高德地图 API =====================
def geocode(city: str, address: str) -> tuple:
    """地理编码：小区名 → 经纬度"""
...
def search_nearby(location: str, poi_type: str, radius: int = 1500) -> list:
    """周边搜索：经纬度 → 附近 POI"""
    params = urllib.parse.urlencode({
        "key": AMAP_KEY,
        "location": location,
...
def get_surrounding_info(city: str, community: str) -> str:
    """获取小区周边配套信息，返回格式化的文本"""
    if not AMAP_KEY:
        return "（未配置地图API，无法自动获取周边信息）"

    location = geocode(city, community)
    if not location:
        return "（未找到该小区位置信息）"
    ...
    categories = {
        "地铁站": "150500",       # 地铁站
        "公交站": "150700",       # 公交站
        "学校(幼儿园/小学/中学)": "141203|141202|141201",  # 幼儿园|小学|中学
        "商场/超市": "060100|060400",  # 商场|超市
        "医院": "090100",         # 医院
        "公园": "110101",         # 公园
        "银行": "160100",         # 银行
    }
    ...
    info_parts = []
    for category_name, type_code in categories.items():
        # 地铁站搜2km，其他1.5km
        radius = 2000 if "地铁" in category_name else 1500
        pois = search_nearby(location, type_code, radius)
        if pois:
            items = []
            for p in pois[:3]:  # 每类最多展示3个
                items.append(f"{p['name']}（约{p['distance']}米）")
            info_parts.append(f"- {category_name}：{'、'.join(items)}")
...
要求：节奏快、信息密度高、适合抖音/快手。挑选1-2个最亮眼的周边配套融入。

### 版本B：60秒中视频（约220-260字）
要求：有故事感、适合小红书/视频号。自然提及周边交通、教育、商业中最突出的2-3个。

### 版本C：3分钟详细讲解（约600-700字）
要求：适合深度介绍，涵盖户型解析、周边配套详细分析、适合人群分析。完整利用周边配套数据。
...
PROMPT_MULTI_PLATFORM = """你是一个多平台内容运营专家。请根据以下房源信息和周边配套，分别生成三个平台的发布文案。
...
- 结构：【开头吸引】+【房源亮点1/2/3】+【周边配套】+【适合人群】+【总结建议】
- 必须包含：话题标签+关键词
...
def main():
    ...
    st.title("🏠 房产 AI 内容助手")
    st.caption("输入房源信息，自动获取周边配套，一键生成口播文案 / 视频标题 / 多平台文案 / 评论区话术")
    ...
    with st.sidebar:
        ...
        if not AMAP_KEY:
            amap_key_input = st.text_input(
                "高德地图 API Key",
                type="password",
                help="在 https://console.amap.com/ 获取"
            )
            if amap_key_input:
                globals()["AMAP_KEY"] = amap_key_input
        else:
            st.success("✅ 地图服务已就绪")
    ...
    # 用 session_state 保存周边信息
    if "surrounding_info" not in st.session_state:
        st.session_state.surrounding_info = ""

    col_btn1, col_btn2 = st.columns([1, 3])
    with col_btn1:
        if st.button("🔍 获取周边配套", use_container_width=True):
            if not community:
                st.error("请先填写小区名称")
            elif not AMAP_KEY:
                st.error("请先配置高德地图 API Key")
            else:
                with st.spinner("正在查询周边配套..."):
                    info = get_surrounding_info(city, community)
                    st.session_state.surrounding_info = info

    # 显示周边信息（可编辑，用户可以手动修正）
    surrounding = st.text_area(
        "周边配套信息（自动获取，可手动修改补充）",
        value=st.session_state.surrounding_info,
        height=150,
        placeholder="点击上方按钮自动获取，或手动填写：如 地铁2号线XX站500米、XX小学对口、XX商场步行10分钟"
    )
    ...
    if st.button("🚀 生成", type="primary", use_container_width=True):
        ...
        params = {
            ...
            "surrounding": surrounding if surrounding else "暂无周边配套信息",
        }
